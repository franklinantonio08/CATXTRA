class Distgastoscuenta {
    constructor() {
    }

    init(){
        
        if($('#gastoscuenta').length) {
          this.gastoscuenta();
      }

      if($('#nuevoregistro').length) {
          this.validategastoscuenta();
      }
      ...
      $('#search').keypress(function(event){
        ...
        gastoscuenta(search){
          ...
          const _this = this
              ...
              const table = $('#gastoscuenta').DataTable( {
                  "destroy": true,
                  "searching": false,
                  "serverSide": true,
                  "info": true,
                  ...
                  "ajax": {
                      ...
                      "data": function ( d ) {
                          var info = $('#gastoscuenta').DataTable().page.info();
          ...
          desactivargastoscuenta(){
              ...
              $( ".desactivar" ).off('click');
                $( ".desactivar" ).click(function() {
                    ...
                    const gastoscuentaId = $( this ).attr( "attr-id" );
                    var opciones = {gastoscuentaId:gastoscuentaId};
                    const message = 'Seguro que desea cambiar de estatus el gastoscuenta?'
                    const objConfirmacionmodal = new Confirmacionmodal(message, opciones, _this.callbackDesactivargastoscuenta);
          ...
          callbackDesactivargastoscuenta(response, opciones){
              ...
              if(response == true){
                  ...
                  {
                      gastoscuentaId: opciones.gastoscuentaId,
                      _token:token 
                      }
                  )
                  .done(function( data ) {
                    ...
                    console.log('por aqui vamos')
                      ...
                      if(data.response == true){
                          const modalTitle = 'GASTOS CUENTA';
                          const modalMessage = 'El gastoscuenta ha sido cambiado de estatus';
                          const objMessagebasicModal = new MessagebasicModal(modalTitle, modalMessage);
                          objMessagebasicModal.init();

                          const objDistgastoscuentas = new Distgastoscuenta();							
                          objDistgastoscuentas.init();	
                          //objDistgastoscuentas.gastoscuentas($( "#search" ).val());

                      }else{
                          
                          const modalTitle = 'GASTOS CUENTA';
                          const modalMessage = 'El gastoscuenta no se ha podido cambiar de estatus';
                          const objMessagebasicModal = new MessagebasicModal(modalTitle, modalMessage);
                          objMessagebasicModal.init();
...
$(document).ready(function(){
  ...
  const objDistgastoscuenta = new Distgastoscuenta();
  objDistgastoscuenta.init();
